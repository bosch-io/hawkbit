...
class DistributionSetManagementSecurityTest
    ...
    protected DistributionSetManagement.Create getCreateObject() {
        return DistributionSetManagement.Create.builder().name("name").version("1.0.0").type("type").build();
    }
    ...
    void invalidatePermissionsCheck() {
        distributionSetTypeManagement.create(DistributionSetTypeManagement.Create.builder().key("type").name("name").build());
        assertPermissions(() -> {
            ((DistributionSetManagement)distributionSetManagement).invalidate(new JpaDistributionSet());
            return null;
        }, List.of(SpPermission.UPDATE_REPOSITORY, SpPermission.READ_REPOSITORY));
    }
}