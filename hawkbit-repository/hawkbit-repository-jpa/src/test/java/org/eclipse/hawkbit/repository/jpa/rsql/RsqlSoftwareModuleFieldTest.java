...
class RsqlSoftwareModuleFieldTest extends AbstractJpaIntegrationTest {
    ...
    void setupBeforeTest() {
        ah = softwareModuleManagement.create(SoftwareModuleManagement.Create.builder().type(appType.getKey()).name("agent-hub")
                .version("1.0.1").description("agent-hub").build());
        softwareModuleManagement.create(SoftwareModuleManagement.Create.builder().type(runtimeType.getKey()).name("oracle-jre")
                .version("1.7.2").description("aa").build());
        softwareModuleManagement.create(
                SoftwareModuleManagement.Create.builder().type(osType.getKey()).name("poki").version("3.0.2").description("aa").build());
        softwareModuleManagement
                .create(SoftwareModuleManagement.Create.builder().type(osType.getKey()).name("*§$%&/&%ÄÜ*Ö@").version("1.0.0")
                        .description("wildcard testing").build());
        softwareModuleManagement
                .create(SoftwareModuleManagement.Create.builder().type(osType.getKey()).name("noDesc").version("noDesc").build());

        final JpaSoftwareModule ah2 = (JpaSoftwareModule) softwareModuleManagement.create(
                SoftwareModuleManagement.Create.builder().type(appType.getKey()).name("agent-hub2").version("1.0.1").description("agent-hub2").build());
